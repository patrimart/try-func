...
function onMessage(message) {
    ...
    function onNext(r, doComplete) {
        if (isComplete)
            return Queue.onFatalException(new Error("Complete has already been invoked."), function () { return process.exit(1); });
        if (r instanceof either_1.Either.Right)
            onNext(r.get(), doComplete);
        else if (r instanceof either_1.Either.Left)
            Queue.onFailure(new ReferenceError("This either is Left."));
