    ...
    function ChildProcess() {
        ...
        this._child.on("message", function (m) {
            if (_this._emitter) {
                ...
                if (m[0]) {
                    _this._emitter(Either_1.default.Left(new Error(m[0])));
                }
                else {
                    _this._emitter(Either_1.default.Right(m[1]));
                }
            }
        });
        this._child.on("error", function (err) {
            _this._isDestroyable = true;
            _this._emitter && _this._emitter(Either_1.default.Left(err));
            _this._child && _this._child.kill();
            _this._child = null;
        });
        this._child.on("exit", function (code, signal) {
            _this._isDestroyable = true;
            _this._emitter && _this._emitter(Either_1.default.Left(new Error("child_process exit(" + code + ", " + signal + ")")));
            _this._child = null;
        });
    }
